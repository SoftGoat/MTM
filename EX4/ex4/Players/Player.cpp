...
bool Player::inputValidation(string name, int &level, int &force, int &hp, int &maxHp, int &coins)
{
    ...
    if (level < MINIMUM_LEVEL || level > MAXIMUM_LEVEL)
    {
        level = DEFAULT_LEVEL;
        unchanged = false;
        // std::cout << "The value of 'level' is invalid value changed to default setting:  " << DEFAULT_LEVEL << std::endl; possible option
    ...
    if (force < MINIMUM_FORCE)
    {
        force = DEFAULT_FORCE;
        unchanged = false;
        //std::cout << "The value of 'force' is invalid, value changed to default setting:  " << DEFAULT_FORCE << std::endl; possible option
        throw std::runtime_error("Invalid Players File");
    }
    ...
    if (maxHp < MINIMUM_HP)
    {
        maxHp = DEFAULT_MAXHP;
        unchanged = false;
        //std::cout << "The value of 'maxHp' is invalid, value changed to default setting:  " << DEFAULT_MAXHP << std::endl; possible option
        throw std::runtime_error("Invalid Players File");
    }
    ...
    if (hp < MINIMUM_HP || hp > maxHp)
    {
        hp = maxHp;
        unchanged = false;
        //std::cout << "The value of 'hp' is invalid, value changed to maxHp value :  " << maxHp << std::endl; possible option
        throw std::runtime_error("Invalid Players File");
    }
    if (coins < MINIMUM_COINS)
    {
        coins = DEFAULT_COINS;
        unchanged = false;
        //std::cout << "The value of 'coins' is invalid, value changed to default setting:  " << DEFAULT_COINS << std::endl; possible option
        throw std::runtime_error("Invalid Players File");
    }
    for (char ch : name){
        if (!isalpha(ch) || name.length() > MAX_NAME_LENGTH || name.length() < MIN_NAME_LENGTH){
            unchanged = false;
            //std::cout << "The value of 'name' is invalid, value changed to default setting:  " << DEFAULT_NAME << std::endl; possible option
            name = DEFAULT_NAME;
            throw std::runtime_error("Invalid Players File");
        }
    }
...
std::shared_ptr<Job> Player::createJob(const std::string& jobName) {
    ...
    // Add more else-if blocks for additional jobs, need to be upadted manually (obviously)

    // Default job if no match found
    //std::cout << "Unknown job, creating default job"; // possible option
    throw std::runtime_error("Invalid Players File");
    return DEFAULT_JOB; 
}
...
std::shared_ptr<Behavior> Player::createBehavior(const std::string& behaviorName) {
    ...
    //std::cout << "Unknown behavior, creating default behavior"; // possible option
    throw std::runtime_error("Invalid Players File");
